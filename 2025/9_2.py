...
for index, i in enumerate(red_points):
	...
	if i[0] == previous_point[0]:
		if abs(i[1] - previous_point[1]) == 1: print("ABORT")
		...
		if abs(i[0] - previous_point[0]) == 1: print("ABORT")
		if i[0] > previous_point[0]:
...
points = set(["%s-%s" % (i[0], i[1]) for i in red_points]).union(set(["%s-%s" % (i[0], i[1]) for i in green_points]))

# TODO 1 : add to green points everything that is "within the grid"
# a point is "within the grid" if we can find a point left, right, up and down that is green ?


# TODO 2 : find all squares with only green and red, and red corners only
# A square is "green or red only" if 0 green point is "within" the square meaning:
# - all vertices are red or green
# - no inner point is green
...
def check_if_correct(i0, i1, j0, j1):
	...
	# 		# print(k0, k1)
	# 		# if (k0 in [i0, j0] or k1 in [i1, j1]) and (k0, k1) not in points:
	# 		# 	print("vertices issues", i0, j0, i1, j1, k0, k1)
	# 		# 	return False
	# 		if (k0 not in [i0, j0] and k1 not in [i1, j1]) and "%s-%s"%(k0, k1) in green_points:
	# 			# print("interior issues", i0, j0, i1, j1, k0, k1)
	# 			return False
	# return True
